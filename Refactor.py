    ...
    if weapon_check(inventory_param):
            ...
            if health_param > 0:
                ...
                if chance_to_run == 0:
                    ...
                    if health_param > 0:
                        ...
                        death(health_param, loop_param, inventory_param, active_effects_param, achievements_param)
        ...
        if chance_to_run == 0:
            ...
            if health_param > 0:
                ...
                print("You died.")
                print("Debug statement - Testing fight_beast")
                # time.sleep(3)
                print("Health: " + str(health_param))
...
def fight_killer(health_param, loop_param, inventory_param, active_effects_param, achievements_param):
    ...
    if invulnerability_check(inventory_param, active_effects_param):
        ...
        # time.sleep(3)
        if weapon_check(inventory_param) and invulnerability_check(inventory_param, active_effects_param):
            # time.sleep(3)
            ...
            while True:
                ...
                if rand_chance == 2:
                    ...
                    if count == killer_run:
                        ...
                        continue
        elif weapon_check(inventory_param) == False and invulnerability_check(inventory_param, active_effects_param):
        ...
        health_param = 0
        loop_param = handle_death(health_param, loop_param)
        return health_param, loop_param
...
# define a function to handle the player's death
...
# it will be assigned to a variable called loop, which will be used to determine if the player is dead or not
# if the player is dead, the loop will be set to 1, and the player will be sent back to the beginning
# if the player is not dead, the loop will be set to whatever current loop the player was at,
# and the player will continue on their journey
...
# wait_function is a function that gives you a random chance of either getting a good event or a bad event
# the good event is max health or a knife
# the bad event encountering the killer or a beast
